package com.ahct.model;

import java.io.Serializable;
import javax.persistence.*;

/**
 * The primary key class for the EHFM_MEDCO_DTLS database table.
 * 
 */
@Embeddable
public class EhfmMedcoDtlsId implements Serializable {
	//default serial version id, required for serializable classes.
	private static final long serialVersionUID = 1L;
	private String medcoId;
	private String hospId;
	//private String scheme;
	private java.util.Date startDt;

    public EhfmMedcoDtlsId() {
    }

	@Column(name="MEDCO_ID")
	public String getMedcoId() {
		return this.medcoId;
	}
	public void setMedcoId(String medcoId) {
		this.medcoId = medcoId;
	}

	@Column(name="HOSP_ID")
	public String getHospId() {
		return this.hospId;
	}
	public void setHospId(String hospId) {
		this.hospId = hospId;
	}
	
	
	
  /*  public String getScheme() {
		return scheme;
	}

	public void setScheme(String scheme) {
		this.scheme = scheme;
	}*/

	@Temporal( TemporalType.DATE)
	@Column(name="START_DT")
	public java.util.Date getStartDt() {
		return this.startDt;
	}
	public void setStartDt(java.util.Date startDt) {
		this.startDt = startDt;
	}

	public boolean equals(Object other) {
		if (this == other) {
			return true;
		}
		if (!(other instanceof EhfmMedcoDtlsId)) {
			return false;
		}
		EhfmMedcoDtlsId castOther = (EhfmMedcoDtlsId)other;
		return 
			this.medcoId.equals(castOther.medcoId)
			&& this.hospId.equals(castOther.hospId)
			&& this.startDt.equals(castOther.startDt);

    }
    
	public int hashCode() {
		final int prime = 31;
		int hash = 17;
		hash = hash * prime + this.medcoId.hashCode();
		hash = hash * prime + this.hospId.hashCode();
		hash = hash * prime + this.startDt.hashCode();
		
		return hash;
    }
}