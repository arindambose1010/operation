...
function fnSearch()
{
	...
	{
	jqueryAlertMsg('Search Criteria Validation',"Please select District also to continue with state type search");
	return false;
	}
	...
	if(document.forms[0].stateType.value!="-1" && document.forms[0].stateType.value=="O"){
		if(parseDate(regFrom)>=parseDate("${bifurcationDate}") || parseDate(regTo)>=parseDate("${bifurcationDate}")){
			alert("Please select from date and to date before "+'${bifurcationDate}'+" for state type Old");
		return false;
		}
		}
		else if(document.forms[0].stateType.value!="-1" && document.forms[0].stateType.value=="N"){
			if(parseDate(regFrom)<parseDate("${bifurcationDate}") || parseDate(regTo)<parseDate("${bifurcationDate}")){
				alert("Please select from date and to date after "+"${bifurcationDate}"+" for state type new");
			return false;
			}
		}
	document.getElementById("district").disabled=false;
...
{
	   document.forms[0].action="./patientDetails.do?actionFlag=casePrintForm&patientId="+patientId+"&caseId="+caseId;
...
function fn_pagination(pageNo,actionType)
{
	...
	var url="./patientDetails.do?actionFlag=getOPCases&actionType="+actionType+"&pageNo="+pageNo+"&patientScheme="+patientScheme;
...
<td colspan="" class="tbcellBorder"><html:select name="patientForm" property="stateType" styleId="stateType" title="Select District Type" onmouseover="addTooltip('state')" onchange="stateSelected()" style="WIDTH: 10em;">
            <html:option value="-1"><fmt:message key="EHF.Select"/></html:option>
...
<td class="tbcellBorder"><html:select name="patientForm" property="district" styleId="district" title="Select District" onmouseover="addTooltip('district')" style="WIDTH: 10em;border:0;">
            <html:option value="-1"><fmt:message key="EHF.Select"/></html:option>
