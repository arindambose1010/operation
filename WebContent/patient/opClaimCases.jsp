...
<!-- Chandana - 8598 - For locking the patientid -->
	...
	<tr>
		...
		<td width="16%" class="tbcellBorder">
	  		<select id="claimsStatus" name="claimsStatus" style="width:180px;">
	  			...
	  			<option value="all">ALL</option><!-- Chandana - 8618 - Added for showing the all option in claim status dropdown for search -->
...
<table id="opClaimCasesTable" width="98%"  cellpadding="1" cellspacing="1" align="center" style="padding-top:0px;margin:0px auto;" class="tb">
	<thead><tr>
		...
		<th class="tbheader1" valign="top" style="width:">Card Number</th><!-- Chandana - 7845 - Added this for showing the card number -->
		<th class="tbheader1" valign="top" style="width:">CR Number</th><!-- Chandana - 7845 - Changed from Card Number to CR Number -->
		<th class="tbheader1" valign="top" style="width:">Patient Name</th>
		<!-- <th class="tbheader1" valign="top" style="width:">Letter Number</th> -->
		<th class="tbheader1" valign="top" style="width:">Claim ORG</th>
	...
	<!-- Chandana - 8598 - Added below scriptlet For getting the blocked cases calling this getBlockedCasesList() method  -->
	<%
    ...
    <tr>
        ...
        <!-- Chandana - 8598 - For showing the lock symbol before the patientid based on the condition -->
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <span style="color:blue;cursor:pointer;"onclick="fn_openOpdClaimCaseDtls('${opClaimCases.PATIENTID}','${opClaimCases.CARDNO}');">${opClaimCases.PATIENTID}</span>
                </c:otherwise>
            </c:choose>
        </td>
        ...
        <td class="tbcellBorder" style="word-wrap:break-word;padding:3px;text-align: center;">${opClaimCases.CLAIMORG}</td>
...
$(document).ready(function(){
	...
	$('#opClaimCasesTable').DataTable({
        ...
        pageLength: 15//Chandana - 8598 - Added this for showing the 15 records per page previosuly it is 10 by default
    });  
...
//Chandana - 8598 - For locking the patient id
 var blockedCases = [<%
        List<String> casesBlocked = ClaimCases.getBlockedCasesList();
...
function fn_search(){
	...
	/* if(parent.parent.globalURl == ''){
		parentSearchUrl = '/Operations/patientDetails.do?actionFlag=getOpClaimCasesList&patientId='+patientId+'&cardNo='+cardNo+'&letterNo='+letterNo+'&caseType='+caseType+'&claimDt='+claimDt+'&claimSubmittedNo='+claimSubmittedNo;
		parent.setGlobalUrl(parentSearchUrl);
	}	
	fn_loadImage();
	document.forms[0].action='/Operations/patientDetails.do?actionFlag=getOpClaimCasesList&patientId='+patientId+'&cardNo='+cardNo+'&letterNo='+letterNo+'&caseType='+caseType+'&claimDt='+claimDt+'&claimSubmittedNo='+claimSubmittedNo;
	document.forms[0].submit(); */
	if(claimDt){
		/* As date is coming in format MM/DD/YYYY to convert it into DD/MM/YYYY */
	...
	$.ajax({
        type: "POST",
        url: "/Operations/patientDetails.do?actionFlag=getOpClaimCasesList&ajaxCall=Y",
        data: {
            patientId: patientId,
            cardNo: cardNo,
            /* letterNo: letterNo, */
        ...
        success: function(data) {
            ...
            if (responseData && responseData.length > 0) {
                ...
                tableHTML += '<th class="tbheader1" valign="top">OP Claim number</th>';
                tableHTML += '<th class="tbheader1" valign="top">Card Number</th>';
                tableHTML += '<th class="tbheader1" valign="top">CR Number</th>';
                tableHTML += '<th class="tbheader1" valign="top">Patient Name</th>';
                /* tableHTML += '<th class="tbheader1" valign="top">Letter Number</th>'; */
                tableHTML += '<th class="tbheader1" valign="top">Claim ORG</th>';
                tableHTML += '<th class="tbheader1" valign="top">Total Amount</th>';
                tableHTML += '<th class="tbheader1" valign="top">Claim Date</th>';
                ...
                $.each(responseData, function(index, opClaimCases) {
                    ...
                    var isBlocked = blockedCases.indexOf(String(opClaimCases.PATIENTID)) !== -1;//Chandana - 8598 - Added this line for getting the blocked cases
                    if(isBlocked){//Chandana - 8598 - Added this condition to show the lock symbol based on the condition
                        ...
                        tableHTML += '<td class="tbcellBorder" style="text-align:center;">' +
                                     '<span style="color:blue;cursor:pointer;" onclick="fn_openOpdClaimCaseDtls(\'' + opClaimCases.PATIENTID + '\', \'' + opClaimCases.CARDNO + '\');">' + opClaimCases.PATIENTID + '</span></td>';
                    }
                    ...
                    /* tableHTML += '<td class="tbcellBorder" style="word-wrap:break-word;padding:3px;text-align: center;">' + opClaimCases.LETTERNO + '</td>'; */
                    tableHTML += '<td class="tbcellBorder" style="word-wrap:break-word;padding:3px;text-align: center;">' + opClaimCases.CLAIMORG + '</td>';
...
function fn_openOpdClaimCaseDtls(patientId, crNo){//Chandana - 8606 - Added CR num in the function to send this cr number to details page
	fn_loadImage();//Chandana - 8598 - Added loading symbol
	if(patientId == null || patientId == ''){
		return false;
	}
	if(patientId != '' && patientId != null){//Chandana - 8598 - Added this to get the flag weather the case got locked ot not
		url = '/Operations/patientDetails.do?actionFlag=getCaseFlag&patientId='+patientId;//Chandana - For Locking the patient id
		$.get(url, function(data){
			if(data == 'yes'){
				fn_removeLoadingImage();
				try{
					if(parent.parent.globalURl == ''){
						parentSearchUrl = '/Operations/patientDetails.do?actionFlag=getOpClaimCaseDtls&patientId='+patientId+'&crNo='+crNo;//Chandana - 8606 - Added cr number to url
						parent.setGlobalUrl(parentSearchUrl);
					}
					fn_loadImage();
				...
				bootbox.alert('This case has already been opened by other user');
				fn_removeLoadingImage();
			}
		}, 'text')
		.fail(function(){
			bootbox.alert('Network error occured.');
			fn_removeLoadingImage();
		});
	}
}
