...
{
	if(caseId=='NA')
		document.forms[0].action="./patientDetails.do?actionFlag=viewPatientDetails&patientId="+patientId;
	else
	   document.forms[0].action="./patientDetails.do?actionFlag=viewPatientDetails&patientId="+patientId+"&caseId="+caseId;
	   document.forms[0].method="post";
	   document.forms[0].submit(); 
}
function printPatient(patientId)
{
	window.open('./patientDetails.do?actionFlag=viewPatientDetails&patientId='+patientId+'&pageType=print','PatientRegPrintPage','left=50,top=50,width=900,height=800,toolbar=no,resize=no,scrollbars=yes');
}
...
function fn_pagination(pageNo,actionType)
{
	...
	var url="./patientDetails.do?actionFlag=ViewRegisteredPatients&actionType="+actionType+"&pageNo="+pageNo+"&stateType=";
...
function resetSearch()
{
	...
	document.getElementById("ageRange").value="-1"; */
...
function stateSelected()
{
    ...
    url = "./patientDetails.do?actionFlag=getLocations&callType=Ajax&lStrHdrId="+lStrHdrId+"&stateId="+state+"&stateType=";
...
<td width="16%" class="labelheading1 tbcellCss"><b><fmt:message key="EHF.HealthCardNo"/></b></td>
...
<td colspan="" class="tbcellBorder"><html:select name="patientForm" property="state" styleId="state" title="Select State" onmouseover="addTooltip('state')" onchange="enableStateType()" style="WIDTH: 15em;">
			...
			<c:forEach items="${stateList}" var="state">
				<c:if test="${state.ID == 'S35'}">
					<html:option value="${state.ID}">${state.VALUE}</html:option>
				</c:if>
			</c:forEach>
    </html:select></td>
...
<th class="tbheader1" style="width:10%">&nbsp;</th>
</tr>
	<logic:iterate name="registeredPatientsList" id="patientVO">
<tr>
<td class="tbcellBorder">&nbsp;
<a href="javascript:openCase('<bean:write name="patientVO" property="patientId"/>','<bean:write name="patientVO" property="caseId"/>')" title="Click On Patient No to view Patient Details"><b><bean:write name="patientVO" property="patientId"/></b></a>
...
<td class="tbcellBorder" align="center"><a href="javascript:printPatient('<bean:write name="patientVO" property="patientId"/>')" title="Click here to get Print Page">Print</a></td>
</tr>
</logic:iterate>
</table>
</logic:notEmpty>
