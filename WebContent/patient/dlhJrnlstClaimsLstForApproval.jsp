	...
	<table id="claimCasesListTable" width="98%"  cellpadding="1" cellspacing="1" align="center" style="padding-top:0px;margin:0px auto;" class="tb">
		<thead>
			<tr>
		    	...
		    	<th class="tbheader1" valign="top" style="width:">Claim Number</th>
				...
				<th class="tbheader1" valign="top" style="width:">Claim Amount</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach var="opClaimCases" items="${opClaimCasesList}" varStatus="loop">
			<tr>
		    	...
		    	<td class="tbcellBorder" style="word-wrap:break-word;padding:3px;text-align: center;">
		    		<span style="color:blue;cursor:pointer;"onclick="fn_openDlhJrnlClaimCaseDtls('${opClaimCases.PATIENTID}','${opClaimCases.CARDNO}','${opClaimCases.ID}');">${opClaimCases.CLAIMNO}</span>
		    	</td>
		        ...
		        <td class="tbcellBorder" style="word-wrap:break-word;padding:3px;text-align: center;">${opClaimCases.HOSPNAME}</td>
...
function fn_search(){
	...
	$.ajax({
        type: "POST",
        url: "/Operations/patientDetailsNew.do?actionFlag=getDlhClaimsforACOApproval&ajaxCall=Y",
        data: {
            patientId: patientId,
            cardNo: cardNo,
            claimDt: claimDt,
...
function fn_openDlhJrnlClaimCaseDtls(patientId, cardNo, type){
	...
	if(patientId == null || patientId == '')
		return false;
	if(parent.parent.globalURl == ''){
		parentSearchUrl = '/Operations/patientDetailsNew.do?actionFlag=getDlhJrnlstClaimCaseDtls&patientId='+patientId+'&cardNo='+cardNo+'&ajaxCall=&acoFlag=Y';
		parent.setGlobalUrl(parentSearchUrl);
	}
	fn_loadImage();
	document.forms[0].action='/Operations/patientDetailsNew.do?actionFlag=getDlhJrnlstClaimCaseDtls&patientId='+patientId+'&cardNo='+cardNo+'&type='+type+'&ajaxCall=Y&acoFlag=Y';
	document.forms[0].submit();
}
...
function fn_buttonClicked(action) {
    ...
    if (confirm("Do You Wish to Verify Selected Cases ?")) {
        ...
        document.forms[0].action="/Operations/patientDetailsNew.do?actionFlag=dlhJrnlstdClaimsBulkApprvl&acoFlag=Y&cases="+cases+"&actionDone="+action;
        document.forms[0].submit();
    } else {
        return false;
    }
