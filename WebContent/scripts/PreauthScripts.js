...
function validateAllphaNumericComb(arg,arg2)
{
    ...
    if(textval != null && textval !='' && textval.charAt(0) !=' ')
    	{
    if(!regExprAlpha.test(textval)){
    	//var fr = partial(focusNClear,textbox1);
		//jqueryAlertMsg('Alphanumeric check','Please enter combination of alphanumeric in  '+arg2,fr);
		alert('Please enter combination of alphanumeric in  '+arg2);
...
//        var Selected=document.forms[0].txt_PHC_mc; 
        ...
        var Dt=trim(selectedvalue.substring((selectedvalue.length)-10));
...
var newWindow;
var urlstring = '/EHF/Common/Calendar.jsp?requestSent='+val;
var urlstyle = 'height=150,width=230,toolbar=no,minimize=no,status=yes,memubar=no,location=no,scrollbars=no,top='+x+',left='+y;
...
function checkAlpha(arg1,arg2) //only alphabets A-Z and a-z
{
     ...
     if(Names1 != null && Names1 != "")
     {
       ...
       if(Names1.search(reg)==-1)
       {
            ...
            focusBox(document.getElementById(arg1));	//k
            return 0;
       }
       return 1;
     }
   ...
   if(ta.value.length > len)
    {
      ...
      // jqueryAlertMsg('Clinical notes fields check',"Please enter only "+ len +" characters in "+arg1);
       ta.value = ta.value.substring(0,len-1);
    }
   /*if (window.event.keyCode == 86)//Paste 
		if(ta.value != null && ta.value !='' && ta.value.length > len)
		 {            
		alert("Please enter only "+ len +" characters in "+arg1);
		 ta.value = ta.value.substring(0,len-1);
		 }*/
...
function checkcontact(arg1,arg)//for accepting numbers either 10 0r 11 digits only
{
	...
	if(len<=9 && len>0)
	{
	   ...
	   focusNClear(arg1);
		...
		//jqueryAlertMsg('Clinical notes mandatory fields',"Enter 10 digits valid "+arg,fr);
		focusBox(document.getElementById(arg1));			//k
	  // arg1.value="";
	 //  arg1.focus();
       return false;	   
	...
	for(i=0,j=0;i<len;i++)
    {
        if((str1.indexOf(str.charAt(i)))==-1)
        {
			...
			focusNClear(arg1);
        	//var fr = partial(focusNClear,arg1);
    		//jqueryAlertMsg('Clinical notes mandatory fields',"Enter Numeric Data in "+arg,fr);
    		focusBox(document.getElementById(arg1));			//k
...
//                arg1.focus();
         ...
         if (i==0)
        	...
        	if(str.charAt(i).search(e))
        		{
        		//var fr = partial(focusNClear,arg1);
        		//jqueryAlertMsg('Clinical notes mandatory fields',arg+" should start with number 7,8 or 9",fr);
...
//        	    arg1.focus();
		...
		{
			//var fr = partial(focusNClear,arg1);
    		//jqueryAlertMsg('Clinical notes mandatory fields',arg +" with all zeros is invalid. Please enter a valid "+arg,fr);
...
function chkAlphaNumeric(arg,arg2)
{
    ...
    if(tbLen.length == 0)
		...
		focusNClear(textbox1);
    	//textbox1.focus();			//k
    	//focusBox(document.getElementById(arg));			//k
//    	textbox1.value='';
    	return false;
    	}
    if(textval.charAt(0)==' '){ 
...
function chkSpecailCharsRemarks(name,remarkLength)
{
    var chars="*|\":<>[]{}`\';$#%&"; //04
    var message1='';
    var msg ='';
