...
function viewCaseDtls(caseNo){
	...
	var url="/<%=context%>/ClaimsFlow.do?actionFlag=getCaseDetails&CaseNo="+caseNo+"&paymentType=ErroneousClaim";
...
function fn_caseSearch()
{
	...
	if(document.getElementById("caseStatus").value=="" ){
		...
		jqueryErrorMsg('Claim Payment Mandatory Field',"Please select Erroneous Claim Status",fr);
		return;
}
	...
	if(document.forms[0].showScheme.value=='show'){
	   	 if(document.forms[0].schemeType.value==''){
	   		 ...
	   		 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId="+document.forms[0].caseStatus.value+"&paymentType=ErroneousClaim&schemeType="+document.forms[0].schemeType.value;
			 document.forms[0].submit();}
		}else{	
			 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId="+document.forms[0].caseStatus.value+"&paymentType=ErroneousClaim";
		     document.forms[0].submit();
		}
...
function fn_reset()
{
	...
	document.getElementById("toDate").value="";
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId=CD109";
     document.forms[0].submit();
}
...
function fn_buttonClicked(buttonId){
	...
	if(checkFlag!=null && checkFlag=='Y'){
	    ...
	    jqueryConfirmMsg('Erroneous Claim Payment Confirmation',displayMsg,fr);	
		}
	else{
    ...
    document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=changeClaimForCases&caseStatus="+document.forms[0].caseStatus.value+"&actionDone="+buttonId+"&paymentType=ErroneousClaim";
    document.forms[0].submit();
}
function checkAll(){
	/*var elements = document.getElementsByClassName("selectableCheckbox");
...
function fn_Refresh()
{
	...
	document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId=CD109&schemeType="+schemeTypeSel;
	document.forms[0].submit();
}
	function exportToExcel() {
		...
		if (document.getElementById("caseStatus").value == "") {
			var fr = partial(focusBox, document.getElementById('caseStatus'));
			jqueryErrorMsg('Claim Payment Mandatory Field',
					"Please select Erroneous Claim Status", fr);
			return;
		}
		...
		document.forms[0].action = "/Operations/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId="
				+ document.forms[0].caseStatus.value
				+ "&paymentType=ErroneousClaim&genFlag=E";
		document.forms[0].submit();
	}
	function exportToPdf() {
		...
		document.forms[0].action = "/Operations/ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId="
				+ document.forms[0].caseStatus.value
				+ "&paymentType=ErroneousClaim&genFlag=P";
		document.forms[0].submit();
	}
</script>
</head>
	...
	<table border="0" width="95%" cellpadding="1" cellspacing="1"
		...
		<tr>
			...
			<table class="tbheader">
				<tr align="center">
					<td colspan="8" align="center" class="tbheader"><b>
					Erroneous Payment </b></td>
				</tr>
			</table>
			</td>
		</tr>
		<tr>
			...
			<table border="1px" width="100%">
				<tr>
					...
					<table width="100%">
						<tr>
							<td width="16%" class="labelheading1 tbcellCss"><b>Erroneous
							Claim Status</b><font color="red">*</font></td>
							<td width="16%" class="tbcellBorder"><html:select
								name="claimsFlowForm" property="caseStatus" styleId="caseStatus"
								style="width:150px;" title="Select Erroneous Claim Status">
							...
							<td width="16%" class="tbcellBorder"><html:text
								name="claimsFlowForm" property="caseId" style="width:150px;"
								maxlength="50" title="Enter Case Number" /></td>
							<td width="16%" class="labelheading1 tbcellCss"><b>Card
							Number</b></td>
							<td width="16%" class="tbcellBorder"><html:text
								style="width:150px;" name="claimsFlowForm" property="wapNo"
						...
						<tr>
							...
							Name</b></td>
							<td width="16%" class="tbcellBorder"><html:text
								name="claimsFlowForm" property="patName" maxlength="50"
								title="Enter Patient Name" /></td>
							<td width="16%" class="labelheading1 tbcellCss"><b>From
							Date</b></td>
							<td width="16%" class="tbcellBorder"><html:text
								name="claimsFlowForm" property="fromDate" styleId="fromDate"
							...
							Date</b></td>
							<td width="16%" class="tbcellBorder"><html:text
								name="claimsFlowForm" property="toDate" styleId="toDate"
		...
		<td width="16%" class="tbcellBorder"><html:select property="schemeType" name="claimsFlowForm" style="width:150px;" title="Please select Scheme">
						...
						<tr>
							...
							<logic:equal value="Y" name="claimsFlowForm" property="genFlag">
								<logic:present name="claimsFlowForm"
									property="casesForPaymentList">
									<bean:size id="listSize1" name="claimsFlowForm"
										property="casesForPaymentList" />
			...
			<logic:equal value="Y" name="claimsFlowForm" property="genFlag">
				<logic:present name="claimsFlowForm" property="casesForPaymentList">
					<bean:size id="listSize" name="claimsFlowForm"
						property="casesForPaymentList" />
					<logic:greaterThan value="0" name="listSize">
						...
						<display:table id="data1" class="displayTable"
							name="claimsFlowForm.casesForPaymentList" pagesize="100"
							decorator="com.ahct.claims.service.ClaimPaymentDecorator"
							style="width:100%;align:center;margin:15px auto;" export="false"
							requestURI="/ClaimsFlow.do" cellpadding="1" cellspacing="1">
							<display:setProperty name="export.pdf" value="false" />
							<display:setProperty name="export.excel" value="false" />
							<display:setProperty name="export.csv" value="false" />
							<display:setProperty name="export.xml" value="false" />
							<display:setProperty name="export.pdf.filename"
								value="ClaimPayments.pdf" />
							<display:setProperty name="export.excel.filename"
								value="ClaimPayments.xls" />
							<display:setProperty name="export.csv.filename"
								value="ClaimPayments.csv" />
							<display:setProperty name="export.xml.filename"
								value="ClaimPayments.xml" />
							<display:column value="<%=i++ %>" title="S No" />
							<display:column style="word-wrap:break-word;padding:3px;" title="Erroneous Claim Id" media="html"
								property="ID" />
							<display:column title="Erroneous Claim Id" media="pdf">${data1.ID}</display:column>
							<display:column title="Erroneous Claim Id" media="excel">${data1.ID}</display:column>
							<display:column title="Case Id" media="csv">${data1.ID}</display:column>
							<display:column title="Case Id" media="xml">${data1.ID}</display:column>
							...
							<display:column style="word-wrap:break-word;padding:3px;" title="Hospital Account Name" property="SUBNAME" />
							<display:column style="word-wrap:break-word;padding:3px;" title="Card Number" property="UNITID" />
							<display:column style="word-wrap:break-word;padding:3px;" title="Claim Amount" property="WFTYPE" />
							<display:column style="word-wrap:break-word;padding:3px;" title="District" property="EMPNAME" />
							<logic:equal value="CD109" property="caseStatus"
								name="claimsFlowForm">
								<display:column style="word-wrap:break-word;padding:3px;"
									title="<input type='checkbox' id='equipNo_id' name='equipNo' title='select All' onclick='javascript:checkAll();' >"
									media="html">
									<html:checkbox name="claimsFlowForm" title="select"
										styleClass="selectableCheckbox" property="caseSelected"
										value="${data1.ID}" styleId="caseSelected" />
								</display:column>
							</logic:equal>
						...
						<table width="100%" align="center" border="0">
							<tr align="center">
								...
								<logic:equal value="CD116" name="claimsFlowForm"
									property="caseStatus">
									<td>
								...
								<logic:equal value="CD196" name="claimsFlowForm"
									property="caseStatus">
									<td>
									<button class="but" type="button" value="viewResFile"
										name="viewResFile" onclick="javascript:fn_attachments('Res');">View
...
{
	var folderFlag = "";
	if(document.forms[0].caseStatus.value=="CD116"){
		folderFlag = "OutFol";
	}
	else if(document.forms[0].caseStatus.value=="CD196"){
		folderFlag = "RecClaimFol";
	}
	var url = "/Operations/attachmentAction.do?actionFlag=onload&UpdType=ehfErr&caseAttachmentFlag=CP&flag="+flag+"&folderFlag="+folderFlag;
