...
<title>RF Claims Payments</title>
...
function fn_caseSearch()
{
	...
	if(document.getElementById("toDate").value!="" && document.getElementById("fromDate").value==""){
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getRfPaymentRecrds";
...
function fn_reset()
{
	...
	document.getElementById("toDate").value="";
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getRfPaymentRecrds&paymentType=CD125";
     document.forms[0].submit();
}
...
function fn_buttonClicked(buttonId){
	...
	if(checkFlag!=null && checkFlag=='Y'){
	    ...
	    jqueryConfirmMsg('RF Payment Confirmation',displayMsg,fr);			
		}
	else{
...
function fn_payNow(buttonId){
	...
	fn_loadImage();
	document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=UpdateRfClaim&actionDone="+buttonId+"&RfStatus="+document.forms[0].caseStatus.value;
    document.forms[0].submit();
}
function checkAll(){
...
function exportToExcel() {
	...
	if(document.getElementById("toDate").value!="" && document.getElementById("fromDate").value==""){
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getRfPaymentRecrds&genFlag=E";
     document.forms[0].submit();
}
function exportToPdf() {
	...
	if(document.getElementById("toDate").value!="" && document.getElementById("fromDate").value==""){
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getRfPaymentRecrds&genFlag=P";
     document.forms[0].submit();
}
</script>
</head>
...
jqueryInfoMsg('RF Claim Information','${saveMsg}',fn_Refresh);
</script>
</c:if>
...
<tr align="center">
<td  colspan="8" align="center"  class="tbheader" ><b> RF Rejected Payment </b>
</td></tr>
</table>
</td></tr>
<tr><td>
	...
	<logic:present name="claimsFlowForm" property="casesForPaymentList">
		...
		<logic:greaterThan value="0" name="listSize">
    		...
    		style="width:100%;align:center;margin:15px auto;"  export="false" requestURI="/ClaimsFlow.do"  cellpadding="1" cellspacing="1">
    			...
    			<display:column   title="RF Payment Id" media="html" property="ID" />
    			<display:column  title="RF Payment Id" media="pdf" >${data1.ID}</display:column>
    			<display:column  title="RF Payment Id" media="excel" >${data1.ID}</display:column>
    			<display:column   title="Case Id" media="html" property="DSGNID" />
    			<display:column  title="Patient Name" property="VALUE"/>
    			<display:column  title="Hospital Name" property="LVL"/>
    			<display:column  title="Card Number" property="UNITID" />
    			<display:column   title="RF Amount" property="WFTYPE" />
    			<display:column title="<input type='checkbox' id='equipNo_id' title='select All' name='equipNo' onclick='javascript:checkAll();' >" media="html"  >
