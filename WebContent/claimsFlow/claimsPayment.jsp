...
function fn_caseSearch()
{
	...
	if(document.forms[0].showScheme.value=='show'){
   	 if(document.forms[0].schemeType.value==''){
   		 ...
   		 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus="+document.forms[0].caseStatus.value+"&schemeType="+document.forms[0].schemeType.value;
		 document.forms[0].submit();}
	}else{	
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus="+document.forms[0].caseStatus.value;
     document.forms[0].submit();
	}
}
function fn_reset()
{
	...
	document.getElementById("toDate").value="";
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus=CD93";
     document.forms[0].submit();
}
...
function fn_buttonClicked(buttonId){
	...
	if(checkFlag!=null && checkFlag=='Y'){
		...
		if (buttonId == '<bean:message key="EHF.Button.Paynow"/>'){
			...
			fr = partial(fn_payNow,buttonId);
		}
		if (buttonId == 'CD73'){
		    displayMsg = 'Do you want to verify';
			fr = partial(fn_verify,buttonId);
		}
		    //alert("Total Cases Selected for Payment : "+caseCount +"\n\nTotal Amount Being Approved : "+totClaimAmt);
...
function fn_verify(buttonId){
	document.getElementById("CD73").style.display="none";
	fn_loadImage();
	
	document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=changeClaimForCases&caseStatus="+document.forms[0].caseStatus.value+"&actionDone="+buttonId;
    document.forms[0].submit();
}
...
function fn_Refresh()
{
	...
	document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus="+caseStatus+"&schemeType="+schemeTypeSel;
	document.forms[0].submit();
}
function exportToExcel() {
	...
	if(document.getElementById("toDate").value!="" && document.getElementById("fromDate").value==""){
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus="+document.forms[0].caseStatus.value+"&genFlag=E";
     document.forms[0].submit();
}
...
function exportToPdf() {
	...
	if(document.getElementById("toDate").value!="" && document.getElementById("fromDate").value==""){
	 ...
	 document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus="+document.forms[0].caseStatus.value+"&genFlag=P";
     document.forms[0].submit();
}
</script>
</head>
...
<tr align="center">
<td  colspan="8" align="center"  class="tbheader" ><b>Claim Payment</b>
</td></tr>
</table>
</td></tr>
<tr><td>
	...
	<logic:present name="claimsFlowForm" property="casesForPaymentList">
		...
		<logic:greaterThan value="0" name="listSize">
			<%int i = 1;%>
					 ...
					 <table id="countTbl" width="80%" align="center">
      		 <tr >
      		 <td  class="labelheading1 tbcellCss" width="33%"><b>Total Number Of Pending Cases:</b></td>
      		 <td  class="tbcellBorder" id="totalCasesPending" width="17%"><b><font color="red">${listSize}</font></b></td>
      		 <td  class="labelheading1 tbcellCss" width="33%"><b>Total Number Of Cases Selected :</b></td>
      		 <td  class="tbcellBorder" id="caseCnt" width="17%"><b><font color="red">0</font></b></td>   
      		
      		 </tr>
      		 
      		  <tr >
      		  <td  class="labelheading1 tbcellCss" width="33%"><b>Total Amount that can be Approved :</b></td>
      		 <td  class="tbcellBorder" id="caseCnt" width="17%"><b><font color="red">Rs ${totalPendingAmt}/-</font></b></td>
      		 <td  class="labelheading1 tbcellCss" width="33%"><b>Total Amount Being Approved :</b></td>
      		 <td class="tbcellBorder" id="totAmt" width="17%"><b><font color="red">Rs 0/-</font></b></td>   		 		 
      		 </tr> 
...
<button class="but"   type="button"  value="viewResFile" name="viewResFile" onclick="javascript:fn_attachments('Res');">View Response File</button>
    		...
    		<display:table  id="data1" class="displayTable" name="claimsFlowForm.casesForPaymentList" pagesize="${pageSize}" decorator="com.ahct.claims.service.ClaimPaymentDecorator" 
    		style="width:auto;align:center;margin:15px auto;table-layout: fixed;"  export="false" requestURI="/ClaimsFlow.do"  cellpadding="1" cellspacing="1">
    			...
    			<display:column value="<%=i++ %>" title="S No" style="width:3%;text-align:center;"/>
    			<display:column  style="word-wrap:break-word; word-break: break-all;padding:3px;width:12%" title="Case Id" media="html" property="ID" />
    			<display:column  title="Case Id" media="pdf" >${data1.ID}</display:column>
    			<display:column  title="Case Id" media="excel" >${data1.ID}</display:column>  
    			...
    			<display:column  style="word-wrap:break-word; word-break: break-all;padding:3px;width:18%" title="Patient Name" property="VALUE"/>
    			...
    			<display:column  style="word-wrap:break-word; break-all;padding:3px;width:12%" title="Status Date" property="LSTUPDDT"/>
    			...
    			<display:column  style="word-wrap:break-word; word-break: break-all;padding:3px;width:5%;text-align:center;" title=" <input type='checkbox' id='equipNo_id' name='equipNo' title='select All' onclick='javascript:checkAll();' >" media="html"  >
                   &nbsp;<input type="checkbox"  title="select"  Class="selectableCheckbox"  name="caseSelected" value="${data1.SUBID}" onchange="checkOrUncheckCase('${data1.SUBID}','${data1.WFTYPE}')" id="caseSelected" />
                </display:column>
                </logic:equal>
                <logic:equal value="CD98" property="caseStatus" name="claimsFlowForm">
    			<display:column  style="word-wrap:break-word; word-break: break-all;padding:3px;width:5%;text-align:center;" title=" <input type='checkbox' id='equipNo_id' name='equipNo' title='select All'  onclick='javascript:checkAll();' >" media="html"  >
                   &nbsp;<input type="checkbox"   title="select" class="selectableCheckbox"  name="caseSelected" value="${data1.SUBID}" id="caseSelected" onchange="checkOrUncheckCase('${data1.SUBID}','${data1.WFTYPE}')" />
                </display:column>
                </logic:equal>
    		    <logic:equal value="CD96" property="caseStatus" name="claimsFlowForm">
    			<display:column  style="word-wrap:break-word; word-break: break-all;padding:3px;width:5%;text-align:center;" title="<input type='checkbox' id='equipNo_id' name='equipNo' title='select All' onclick='javascript:checkAll();' >" media="html"  >
                   <input type="checkbox"  Class="selectableCheckbox"  title="select" name="caseSelected" value="${data1.SUBID}" id="caseSelected" onchange="checkOrUncheckCase('${data1.SUBID}','${data1.WFTYPE}')" />
                </display:column>
                </logic:equal>
      		 </display:table>
