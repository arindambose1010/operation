...
<link href="css/themes/<%=themeColour%>/commonEhfCss.css" rel="stylesheet" type="text/css" media="screen">
	...
	<script src="js/jquery-1.9.1.mini.js" type="text/javascript"></script>
...
function fn_sendBack(buttonId){
	...
	//setTimeout("ceoRemark.focus()", 0);  
	//alert("Please Enter Remarks \n");
	jqueryErrorMsg('Claim Mandatory Fields',"Please Enter Remarks <br>",fr);
	return;
	}
	var displayMsg='';
	var caseNo= '${patCommonDtls.CASENO}';
	if (buttonId == '<fmt:message key="EHF.Button.SentBackToCH"/>')
		displayMsg = '<fmt:message key="EHF.Button.Msg.SentBackToCH"/>';
	var fr = partial(fn_sendBackNow,buttonId,caseNo);
    jqueryConfirmMsg('Send Back Confirmation',displayMsg,fr);		
}
function fn_sendBackNow(buttonId,caseNo){
	document.forms[0].action="/<%=context%>/ClaimsFlow.do?actionFlag=sendBackToCh&caseId="+document.forms[0].caseId.value+"&actionDone="+buttonId+"&CaseNo="+caseNo;
...
function maxLengthPress(field,maxChars,e)
{
	...
	{
	jqueryErrorMsg('Max Length Validation',"Remarks length should not excede " +maxChars ,fr);
	//alert("Remarks length should not excede " +maxChars );
...
function checkCochlear()
{
	var cochlearFlag='${cochlearFlag}';
	if(cochlearFlag=='Y')
		{
			document.getElementById("remarksTbl").style.display="none";
			document.getElementById("buttonsDiv").style.display="none";
			document.getElementById("closeDiv").style.display="";
		}
	else
		{
			document.getElementById("remarksTbl").style.display="";
...
<body onload="javascript:checkCochlear()">
<form action="/ClaimsFlow.do" method="post" >
<c:if test="${saveMsg ne null}">
<script>
var fr = partial(window.opener.location.reload());
...
<td width="25%"  align="left" class="tbcellBorder">${patCommonDtls.VILLAGE}</td>
 <td width="10%" class="labelheading1 tbcellCss" align="left"><b>Registration Date</b></td>
<td width="25%"  align="left" class="tbcellBorder">${patCommonDtls.regdate}</td>
</tr>

 <tr align="center"><td width="15%" class="labelheading1 tbcellCss" align="left"><b>Surgery/Therapy Date</b></td>
 <td width="20%" align="left" class="tbcellBorder">${patCommonDtls.surgeryDate}</td>
 <td width="10%" class="labelheading1 tbcellCss" align="left"><b>Discharge/Death Date</b></td>
<td width="25%"  align="left" class="tbcellBorder">${patCommonDtls.disDate}</td>
...
<tr align="center"><td width="15%" class="labelheading1 tbcellCss" align="left"><b>Hospital Name</b></td>
...
<td width="15%" class="labelheading1 tbcellCss" align="left"><b>Erroneous Claim Id</b></td>
<td width="20%"  align="left" class="tbcellBorder">${patCommonDtls.telephonicId}</td>
</tr>
<tr align="center"><td width="15%" class="labelheading1 tbcellCss" align="left"><b>Erroneous Claim Status</b></td>
 <td width="20%" align="left" class="tbcellBorder">${patCommonDtls.comorbidName}</td>
</tr>
 </table></td></tr>
...
<td style="word-wrap:break-word;"  class="tbcellBorder"><bean:write name="results1" property="auditDate" /></td>
...
<logic:equal value="GP40" property="roleId" name="claimsFlowForm">
<table width="100%" align="center" id="remarksTbl"> 
	<tr ><td  class="labelheading1 tbcellCss"><fmt:message key="EHF.Claim.DocVer6"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <html:textarea name="claimsFlowForm" property="ceoRemark"  styleId="ceoRemark" style="WIDTH:80%;height:3em" onkeydown="return maxLengthPress(this,3000,event)" onkeypress="return validateSplKeyCodes(event);" onchange="validateSpaces(this,'Ceo Remarks')" title="Enter Remarks"/>
    </td></tr>
 </table>
 </logic:equal>
...
<tr><td>&nbsp;</td></tr>
<tr  align="center"><td  id="buttonsDiv">
