...
<head>
	...
	<title>View Death Cases Bootstrap</title>
...
function viewPreviousPages(pageNo,noOfPages,selectedPage,end_index)
{
	...
	if(minPageNo>1)
	{
		pageDisplays=pageDisplays+'<li><a href="javascript:viewPreviousPages('+pageNo+','+noOfPages+','+selectedPage+','+end_index+')"><span class="glyphicon glyphicon-backward" style="cursor: pointer;"></span></a></li>';
	...
	pageDisplays=pageDisplays+'<li><a href="javascript:viewNextPages('+pageNo+','+noOfPages+','+selectedPage+','+end_index+')"><span class="glyphicon glyphicon-forward" style="cursor: pointer;"></span></a></li></ul>';
...
function viewNextPages(pageNo,noOfPages,selectedPage,end_index)
{
	...
	function exportToCSV()
	{
		...
		id.onclick=null;
		document.forms[0].action="/Operations/casesSearchAction.do?actionFlag=viewDeathCases&csvFlag=Y&searchFlag=Y";
		document.forms[0].method="post";
		document.forms[0].submit();
	}
...
<li class="lispacing">${start_index}  - ${endresults} of ${noOfRecords} </li>
</ul></div>
<div class="centerone" id="pageNoDisplay">
<ul class="pagination"> 
...
for(int i=minVal;i<=maxVal;i++)
{
	pageNo=i;
		...
		{
			%>
			<li><a href="javascript:fn_pagination(<%=pageNo%>,<%=end_index%>)"><b><%=pageNo%></b></a></li>
			<%
		}
}
if(pageNo<noOfPages)
	{
		%>
		<li><a href="javascript:viewNextPages(<%=pageNo+1%>,<%=noOfPages%>,<%=selectedPage%>,<%=end_index%>)"><span class="glyphicon glyphicon-forward" style="cursor: pointer;"></span></a></li>
		<%
	}
		%>
<logic:notEmpty name="casesSearchFormClaims" property="prev"> 
<li><span class="glyphicon glyphicon-backward" onclick="javascript:showPagination('<bean:write name="casesSearchFormClaims" property="prev" />')"></span></li>
</logic:notEmpty>
<logic:notEmpty name="casesSearchFormClaims" property="next">
<li><span class="glyphicon glyphicon-forward" onclick="javascript:showPagination('<bean:write name="casesSearchFormClaims" property="next"/>') "></span></li>
</logic:notEmpty>
</ul></div> 
<div class="rightone">
<ul class="pagination">
<li class="lispacing">Show in sets of</li>
<c:if test="${end_index ne 10}"><li><a href="javascript:fn_pagination(1,10)">10</a></li></c:if>
...
<c:if test="${end_index eq 100}"><li class="active"><span>100</span></li></c:if>
</ul></div>	
...
<script>parent.fn_removeLoadingImage();
