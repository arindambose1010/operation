...
<%--
    indexOpenAM.jsp contains links to test SP or IDP initiated Single Sign-on
--%>
<%
    ...
    if ((fedletHomeDir == null) || (fedletHomeDir.trim().length() == 0)) {
        if (System.getProperty("user.home").equals(File.separator)) {
            fedletHomeDir = File.separator + "Operations";
        } else {
            fedletHomeDir = System.getProperty("user.home") +
                File.separator + "Operations";
        }
    }
%>
<html>
...
<%
    ...
    try {
        if (createConfig) {
                ...
                try {
                    ...
                    if (src != null) {
                        ...
                        throw new SAML2Exception("File " + source + 
                            " could not be found in Operations.war");
                    }
                } catch (IOException e) {
            ...
            out.println("<br><br>Click <a href=\"indexOpenAM.jsp\">here</a> to continue.");
        } else {
            ...
            if (!dir.exists() || !dir.isDirectory()) {
                ...
                if (confExist) {
                    out.println("<br><br>Click <a href=\"indexOpenAM.jsp?CreateConfig=true\">here</a> to create Fedlet configuration automatically.");
                ...
                if ((spEntityID == null) || (idpEntityID == null)) {
                    ...
                    if (confExist) {
                        out.println("<p><br>Click <a href=\"indexOpenAM.jsp?CreateConfig=true\">here</a> to create Fedlet configuration automatically.");
                        out.println("<br>Or manually extract your fedlet.war and copy all files under \"conf\" directory to \"" + fedletHomeDir + "\" directory, then restart your web container.");
                    } else {
                        out.println("<br>Please follow the README bundled inside your Fedlet-unconfigured.zip file to setup Fedlet configuration, then restart your web container.");
                    }
...
%>
	<META HTTP-EQUIV="Refresh" CONTENT="0; URL=<%= idpBaseUrl %>/idpssoinit?NameIDFormat=urn:oasis:names:tc:SAML:2.0:nameid-format:transient&metaAlias=<%= idpMetaAlias %>&spEntityID=<%=spEntityID %>&binding=urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Artifact">
    <%-- <tr>
      <td colspan="2"> </td>
    </tr>
     <tr>
