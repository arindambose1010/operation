...
{
	document.forms[0].action="./medicalAudit.do?actionFlag=getSampleCasesForAudit&pageId="+pageId+"&end_index="+end_index+"&backFlag=${backFlag}";
...
<%-- document.forms[0].action = '/<%=context%>/medicalAudit.do?actionFlag=medicalAuditSearchPage';
document.forms[0].submit(); --%>
 }
 ...
 function mandatoryChk(){
	 if(document.getElementById('selectionType').value == ""){
		...
		// jqueryAlertMsg("Alert","Please select High Volume/High Cost in Selection Type");
		 alert("Please select High Volume/High Cost in Selection Type");
		 return false;
	 }
	 return true;
 } 
 function getSampleCases(){
	 ...
	 var verify=mandatoryChk();
	 if(verify == true){
	 fn_loadImage();
	 document.forms[0].action = '/<%=context%>/medicalAudit.do?actionFlag=getSampleCasesForAudit&backFlag=no';
	 document.forms[0].submit(); }
 ...
 function showCase(caseId){
		...
		document.forms[0].action ='/<%=context%>/casesApprovalAction.do?actionFlag=getCaseDtls&CaseId='+caseId+'&caseSearchType=caseIp&casesForApproval=N&ipOpType=IP&errSearchType=N&disSearchType=N&module=medicalAudit';
		document.forms[0].submit();
	}
...
<body onload="fn_removeLoadingImage(),fn_changeSpecs('Y')">
...
<table id="Search" align="center"  width="90%" cellpadding="0" cellspacing="0" class="tb" style="padding-top:0px;margin:0px auto;">
						...
						<html:select property="selectionType" name="medicalAuditForm" style="width:180px" title="Selection Type" onclick="javascript:fn_changeSpecs('N')" onmouseover="addTooltip('selectionType')" styleId="selectionType">
...
<%int i = ((Integer) request.getAttribute("start_index")).intValue()+1;%> 
 <%--
  ...
  <display:table style="padding-left:20px"   name="medicalAuditForm.auditList" id="rowId" requestURI="/medicalAudit.do"  pagesize="5" export="false">
				...
				<display:setProperty name="basic.show.header">false</display:setProperty>
							<thead> --%>
								...
								<td class="tbheader1" ><b>SNO</b></td>
									...
									<td class="tbheader1" ><b>COST AMOUNT</b></td>
									</c:if>
									<c:if test="${selectType eq 'PHV'}">
									<td class="tbheader1" ><b>PROCEDURE</b></td>
									</c:if>
									</tr>
               ...
               <td class="tbcellBorder"><a href="javascript:showCase('${rowId.CASEID}');" title="Click to View Case Details"><c:out value="${rowId.CASENO}"/></display:column></a>
               ...
               <c:if test="${selectType eq 'PHC'}">
               <td class="tbcellBorder"><bean:write name="auditList" property="CLAIMAMOUNT"/>	</td>
               </c:if>
               <c:if test="${selectType eq 'PHV'}">
               <td class="tbcellBorder"><bean:write name="auditList" property="PROCEDURENAME"/>	</td>
				</c:if>
...
<!-- </td>
	  ...
	  <div id="processImagetable" style="top:60%;width:80%;padding-left:10em;z-index:50;height:10%;position:absolute;">
